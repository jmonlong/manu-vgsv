...
identity %>%
  ...
  geom_point(size=4) +
  labs(color="Strain", shape="Clade", x="Average mapping identity of short reads on sample graphs (from construct graph)", y="Average mapping identity of short reads on sample graphs (from cactus graph)") +
  coord_cartesian(xlim=c(0.6,1), ylim=c(0.6,1)) +
  geom_abline(intercept=0) +
  theme_bw() +
  theme(legend.position=c(.99,.01), legend.justification=c(1, 0),
...
quality %>%
  ...
  ggplot(aes(construct, cactus, color=strain, shape=clade)) +
  geom_point(size=4) +
  labs(color="Strain", shape="Clade", x="Average mapping quality of short reads on sample graphs (from construct graph)", y="Average mapping quality of short reads on sample graphs (from cactus graph)") +
...
score %>%
  ...
  ggplot(aes(construct, cactus, color=strain, shape=clade)) +
  geom_point(size=4) +
  labs(color="Strain", shape="Clade", x="Average alignment score of short reads on sample graphs (from construct graph)", y="Average alignment score of short reads on sample graphs (from cactus graph)") +
