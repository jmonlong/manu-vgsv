...
data <- read_tsv("data/yeast/four.strains.ids_and_mapqs.mean.tsv", col_names=c("strain", "cactus_id", "vcf_id", "linear_id", "cactus_mapq", "vcf_mapq", "linear_mapq"))
...
data %>%
  ...
  ggplot(aes(vcf_id, cactus_id, color=strain, alpha=ingraph, shape=clade)) +
  geom_abline(intercept=0) +
  geom_point(size=4) +
...
data %>%
  mutate(clade=ifelse(strain %in% c("UWOPS919171","UFRJ50816","YPS138","N44","CBS432"), 'paradoxus', 'cerevisiae')) %>%
  mutate(ingraph=ifelse(strain %in% c("UFRJ50816", "YPS128", "CBS432", "SK1", "S288c"), 'included', 'excluded')) %>%
  ggplot(aes(vcf_mapq, cactus_mapq, color=strain, alpha=ingraph, shape=clade)) +
  geom_abline(intercept=0) +
  geom_point(size=4) +
  geom_label_repel(aes(label=strain),
                   point.padding=.25, box.padding=.5,
