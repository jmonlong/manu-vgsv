...
pr.df = pr.df %>% filter(type!='INV', type!='Total',
...
ps.df = ps.df %>% group_by(experiment, type, size, method, region, eval) %>%
  select(TP, TP.baseline, FN, FP) %>% summarize_all(sum)
ps.df = prf(ps.df)

pdf('pdf/hgsvc-giab-chmpd-svpop-call-persize.pdf', 9, 7)

ps.df %>% filter(eval=='presence', type!='INV') %>% ungroup %>% 
