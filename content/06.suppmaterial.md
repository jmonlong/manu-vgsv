...
[//]: # (I think all tables with best F1 scores and figures with precision-recall curves should mention what was used for thresholding. I assume for the calling evaluation that variant quality was used, but for the genotype evaluation it is not mentioned whether genotype quality or something else was used.)
...
[//]: # (I think the precision-recall curves would be easier to interpret if the limit (size) of each axis were the same. Right now it is a bit hard to interpret the trade-off of each method. In my opinion they to not necessary have to go from 0-1.)
...
[//]: # (It seems like this figure does not exist anymore.)
