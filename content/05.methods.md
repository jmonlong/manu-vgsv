...
[//]: # (I am still a bit confused about this. Is it only the truth set variants where the phase might be known that are merged? If it is also the variants in the call set I would be a bit worried since they might not be phased, for example BayesTyper will alwayes give 0/1 and never 1/0. If call set variants are also merged, I think it is something that will need to be changed at some point.)
...
[//]: # (How was non-repeat regions classified for figure 2 etc.? In the same manner as described for the SMRT-SV2 Comparison? I think it should also be written here since it is also used for the other benchmarks.)
...
[//]: # (It might be good to have a link to the file on the FTP site here, since the data is not published yet.)
...
[//]: # (I think it is important to write how the strains used for construction was selected. Randomly?)
...
[//]: # (Did you do any filtering of the variants detected by each method? I seem to remember that AsmVar detects all types of variation including SNVs. Or am I misremembering?)
...
[//]: # (Maybe mention the difference in how variants was called on the cactus and vcf graphs (with and without recall option).)
