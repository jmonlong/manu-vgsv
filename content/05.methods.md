...
[//]: # (I am still a bit confused about this. Is it only the truth set variants where the phase might be known that are merged? If it is also the variants in the call set I would be a bit worried since they might not be phased, for example BayesTyper will alwayes give 0/1 and never 1/0. If call set variants are also merged, I think it is something that will need to be fixed at some point.)
