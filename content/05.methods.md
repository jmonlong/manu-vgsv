...
[//]: # (I am still a bit confused about this. Is it only the truth set variant where the phase might be known that are merged? If it is also the variants in the call set I would be a bit worried since they are normally not phased, for example BayesTyper will alwayes give 0/1 and never 1/0.)
