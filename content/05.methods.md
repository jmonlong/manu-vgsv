...
Additional File 1: Fig. {@fig:sveval} shows an overview of the SV evaluation approach which is described below.
...
The results shown in this study used a minimum of 50% coverage to match variants but we also replicated the results using 90% minimum coverage and observed similar results (see Additional File 1: Fig. {@fig:eval-stringent}). 
...
The average number of genotyped variants per variant in the truth set (Additional File 1: Fig. {@fig:eval-matchtp}) was computed by dividing the number of TPs from the call set by the number of TPs from the truth set, i.e. the ratio of matched variants between the two variant sets.
...
Running times and memory usage for the different tools are shown in Additional File 1: Table {@tbl:timing}.
...
We also explored the performance of vg and SMRT-SV v2 Genotyper in different sets of regions (Additional File 1: Fig. {@fig:svpop-regions} and Additional File 1: Table {@tbl:svpop-regions}):
...
For the analysis of graphs from *de novo* assemblies, we utilized publicly available PacBio-derived assemblies and Illumina short read sequencing datasets for 12 yeast strains from two related clades (Additional File 1: Table {@tbl:strains}) [@doi:10.1038/ng.3847].
