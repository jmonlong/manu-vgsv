...
Some SV genotyping methods require additional information or special VCF formatting [@doi:10.1101/558247].
...
Nebula, a new k-mer-based genotyper, requires reads from an individual with the SV during k-mer selection[@doi:10.1101/566620].
In contrast, vg can take as input either explicit or symbolic VCFs, as well as assembly alignment, and is thus much simpler to set up and more broadly applicable than some comparable tools.
...
While this is sufficient to represent single copy changes, such as deletions or single tandem duplications, CNVs with multiple copy number states are not addressed by the current implementation.
