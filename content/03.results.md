...
b) Simulation experiment. Each subplot shows a comparison of genotyping accuracy for four SV calling methods. Results are separated between types of variation (insertions, deletions, and inversions). The experiments were also repeated with small random errors introduced to the variant locations to simulate breakpoint uncertainty (1-10bp). For each experiment, the y-axis shows the maximum F1 across different minimum quality thresholds.
SVTyper cannot genotype insertions, hence the missing line in the top panels.
[//]: # (I think it would be better if SVTyper was removed from the insertion plots, similar to figure 2.)
](images/panel1.png){#fig:1}
[//]: # (You use best F1 in figure 2 and F1 here.)
...
[//]: # (We are only talking about brakpoint location right? And not mutations around the breakpoint?)
...
In non-repeat regions and across the whole genome, the F1 scores and precision-recall trade-off were better for vg compared to other methods. 
...
We evaluated the SV genotyping methods on this sample using the GIAB callset as input.
This callset contains variants from both HG002 and his parents (HG003 and HG004).
...
[//]: # (It is a bit unclear why you only compare against SMRT-SV2 in this section and not delly etc.)
...
The first graph (called *VCF graph* below) was created from the linear reference genome of the *S.c. S288C* strain and a set of SVs relative to this reference strain in VCF format identified from the assemblies by three methods: Assemblytics [@doi:10.1093/bioinformatics/btw369], AsmVar [@doi:10.1186/s13742-015-0103-4] and paftools [@doi:10.1093/bioinformatics/bty191].
...
[//]: # (It sounds from this that you are only using SVs to construct the sample graphs. My understanding from the methodds section was that the sample graphs also contained SNVs and indels.)
