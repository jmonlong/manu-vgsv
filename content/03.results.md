...
[//]: # (The caption in the figure says that the real read experiment included HG00514, HG00733, and NA19240. Here you say only HG00514. You should make sure that whichever one is incorrect is fixed up.)
...
In non-repeat regions and across the whole genome, the F1 scores and precision-recall AUC (data not shown) were higher for vg compared to other methods. 
...
[//]: # (I'm unclear whether this experiment uses the same graphs/VCFs as the previous ones or whether you're using their high quality call set. I'm editing it assuming the former, so we might need to remove some of the new material if I'm wrong.)
...
We evaluated the SV typing methods on this sample as well.
Unlike in previous experiments, the sample we were typing was not included in the catalog of SVs we used.
This experimental design more accurately represents many practical research settings. 
...
[//]: # (Just to be sure, is it actually HG5014 or is it supposed to be HG00514?)
...
Furthermore, genome alignments from graph-based software such as Cactus [@doi:10.1101/gr.123356.111] can contain complex structural variation that is extremely difficult to represent, let alone call, outside of a graph.
...
[//]: # (The description of the experimental set up in this paragraph is unclear to me. I've tried to clean it up, but I'm not totally sure I understood it well enough to do it accurately.)
We analyzed public sequencing datasets for 12 yeast strains from two related clades (*S. cerevisiae* and *S. paradoxus*) [@doi:10.1038/ng.3847].
...
The first graph (called *VCF graph* below) was constructed by calling SVs in the VCF relative to the *S.c. S288C* and then adding these SVs onto it. 
The SVs were identified using three methods: Assemblytics [@doi:10.1093/bioinformatics/btw369], AsmVar [@doi:10.1186/s13742-015-0103-4] and paftools [@doi:10.1093/bioinformatics/bty191].
The second graph (called *cactus graph* below) was derived from a multiple genome alignment of the five strains using our Cactus tool [@doi:10.1101/gr.123356.111].
...
[//]: # (I think this mapping comparison could probably be moved to the supplement. The rest of the main text is very focused on SV typing. I personally find it a bit confusing when I suddenly start seeing mapping results. It's good that we thought about this, but perhaps we could just mention the headline results in a sentence and refer people to the supplement. What do you guys think? I also think that we could do away with the MAPQ plot. To my mind, identity is a better measure for what we're trying to show, and it seems to make the point even more strongly.)
![**Mapping comparison.** 
...
[//]: # (How do we compute genetic distance between the strains here? Also, have we looked at a scatterplot to make sure the linearity/homoskedasticity assumptions for the Pearson test are justified? Do we get similar results with a Spearman test? If so, that's much more robust to nonlinear relationships and non-normal data.)
...
The *VCF graph* only exhibited higher mappability on the reference strain *S.c. S288C*, and the difference was marginal.
We also found that the genetic distance to the reference strain correlates with the better mapping on the *cactus graph* over the *VCF graph* (Pearson's product-moment correlation, p-value=1.415e-11).
...
We observed similar trends in the mapping identity of the short reads as with mapping quality (see Figure {@fig:3}b and [Supplementary Information](#supplementary-information)).
...
Only the number of alignments with perfect identity is substantially lower for the strains that were not included in the creation of the graphs (see Figure {@fig:3}b). 
...
[//]: # (Here too I think identity is better than MAPQ as a metric. With MAPQ you have confound of repetitiveness, where correctly adding repetitive sequence could end up lowering your MAPQ and making you look worse. What's the reasoning for including both MAPQ and identity? If we have a strong reason for it, we should make it explicit in the text because it's not clear to me.)
...
If a given call set is correct, we expect that reads from the same sample will be mapped confidently and with high identity to the corresponding sample graph.
...
Again, our results did not show a large difference between strains included in the graph and those that were excluded. 
...
More accurate genotypes should result in reference graphs that have good mappings for a greater proportion of the reads.
