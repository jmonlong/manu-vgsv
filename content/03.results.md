...
While vg outperformed Delly and SVTyper, BayesTyper showed the best F1 score and precision-recall trade-off (Figure {@fig:2} and {@fig:hgsvc-sim-geno}, Table {@tbl:hgsvc}).
...
Here vg clearly outperformed other approach, most likely because of its graph-based strategy and robustness to errors in the SV catalog ({@fig:2} and  {@fig:hgsvc-real-geno}).
...
We first called SVs from the pseudo-diploid genome and reads constructed by merging datasets from two haploid cell lines and used to train SMRT-SV2[@tag:audano2019].
...
Of note, Audano et al. had identified 27 inversions, 7 of which were predicted correctly by vg.
