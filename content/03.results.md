...
To specifically quantify mappability in SV regions we excluded reads that produced identical mapping quality and percent identiy on both sample graphs and an empty sample graph containing the linear reference only (see [Methods](#calling-and-genotyping-of-svs)).
Then, we compared the average percent identity and mapping quality of the remaining short reads on each sample graph (Figures {@fig:4}a and b).
For 10 of the 11 strains, we observed higher average percent identity of the short reads on the *cactus sample graph* compared to the *VCF sample graph* (mean improvement in percent identity = 8.5%).
Only for the YPS128 strain, reads on the *VCF sample graph* reached a slightly higher percent identity.
Generally, the improvement in percent identity between both graphs was larger for strains in the *S. paradoxus* clade (between 7% and 20%) than for strains in the *S. cerevisiae* clade (between 1% and 12%).
We also observed generally lower percent identity for strains in the *S. paradoxus* clade on both sample graphs.
While the higher percent identity indicated that the *cactus graph* represents the reads better (Figures {@fig:4}a), the higher mapping quality confirmed that this did not come at the cost of added ambiguity or a more complex graph (Figures {@fig:4}b).
For all strains, we observed higher average mapping quality of the short reads on the *cactus sample graph* compared to the *VCF sample graph* (mean improvement in mapping quality = 5.7).
Our results did not reveal a substantial difference between strains included in the graph and those that were excluded.
...
The short reads were aligned to the sample graphs and reads with identical mapping quality and percent identity across both sample graphs and an additional empty sample graph were removed from the analysis.
Then, the quality of the remaining alignments was used to ascertain SV genotyping performance.
